...
模型插件声明是否支持图片、工具调用、JSON 输出、推理深度、缓存用量字段、模型列表接口和流式输出。中心服务基于模型能力判断附件和工具是否可用，前端不能自行猜测。
...
`workspace` 对话页输入区保存当前会话的模型发送配置 UI 状态，字段包括模型名称、推理深度和执行模式。供应商 ID 可以作为模型来源和中心服务发送设置保存在状态容器中，但输入框内部不展示供应商选择控件。默认值从中心服务返回的启用供应商、供应商默认模型、已保存模型列表和推理深度配置中取值；供应商没有模型列表或刷新失败时，模型名称允许由用户手动输入或创建。该选择只影响后续发送和后续任务，不回改历史消息、历史任务、历史用量或记忆；在中心服务执行链路真正消费该配置前，前端不得破坏现有发送协议。
...
页面级路由必须全部使用懒加载，不能在路由文件静态导入 `views` 页面组件。Vite 构建必须通过 `manualChunks` 把 Vue 运行时、UI 框架、Markdown 渲染库和其他三方依赖拆出主入口文件，避免 `index.html`、`plugin.html` 共用入口的首屏 JS 过大。
