...
`apps/frontend` 是唯一业务前端。它使用 Vue 3、Vite、Vue Router、Pinia、Element Plus 和 Vant，构建 `index.html` 和 `plugin.html` 两个入口。

`apps/desktop-shell` 是 Electron 壳。它只负责窗口、托盘、系统通知权限检测、中心服务生命周期、本机配置入口、绿色版 zip 打包和前端资源承载。桌面壳不保存供应商、会话、记忆、任务等核心事实。

`services/center` 是中心服务。它使用 Node.js、TypeScript、Fastify 和 better-sqlite3，包含领域模块、REST API、WebSocket、数据仓储、事件日志、Worker 管理、访问控制和扩展运行管理。
