...
- `设计.md` 保存当前功能设计和验收设计；执行前必须评估并同步本轮设计，不能把多个功能、多个端或多个验收口径揉在同一个设计标题下。
- `设计.md` 必须按每一次设计拆分一级标题；每个一级标题必须是明确的设计名称，不能只写“设计”“调整”“优化”这类泛称。
- `设计.md` 每个设计标题下必须至少包含目标、范围、设计要点、验收口径和清单状态，必要时再补充交互、数据流、接口、边界和风险。
- `设计.md` 的目标必须写清楚该设计要解决的问题和完成后的可验证结果；范围必须写清楚本轮包含什么、不包含什么，避免执行时扩大实现边界。
- 执行时如果当前设计与 `设计.md` 中已有设计或用户之前明确提出的设计冲突，必须先停下来告知冲突点，让用户决定采用哪个口径；用户确认后再修订 `设计.md`、`需求.md` 或 `架构.md` 并落代码。
...
- 三目运算符只允许在简单变量赋值的时候使用，且不允许嵌套使用，可读性极差
- 复杂功能用class组织代码，方便复用和管理
...
- 优先使用ideaMCP服务检索代码
- 代码组织要按功能模块化，比如代码工具就全部放到uitls文件夹中，智能体定义就放到agents里面，智能体工具就放到tools里面

## 测试规范

- ts源码不允许编译再启动，项目开发启动是已经可以直接启动的
- 使用用户环境里面的node启动
...
以下行为准则用于减少 LLM 编码时的常见错误，来自 [Andrej Karpathy 关于 LLM 编码陷阱的观察](https://x.com/karpathy/status/2015883857489522876)。

**取舍：** 这些准则会让执行更偏谨慎而不是更快。遇到琐碎任务时，按实际情况判断。
...
- 明确说明你的假设。如果不确定，就询问。
- 如果存在多种理解，列出来，不要暗自选择。
- 如果存在更简单的方案，直接说明。必要时提出异议。
- 如果有不清楚的地方，停下来，说清楚哪里不明确，然后询问。
...
如果你的改动产生了孤立内容：
- 删除由你的改动造成的无用导入、变量或函数。
- 不要删除原本就存在的死代码，除非用户要求。

检验标准：每一行改动都必须能直接追溯到用户请求。
...
强成功标准可以让你独立循环推进。弱成功标准（例如“让它能用”）需要不断澄清。
