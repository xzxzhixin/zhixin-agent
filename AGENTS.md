...
- `需求.md` 和 `架构.md` 必须随着最近代码和真实实现持续保持一致；每次完成代码修改、规则调整、架构调整或验收口径调整后，都要评估这两个文件是否需要同步。
- 代码实现已经改变产品能力、交互口径或验收口径时，必须同步更新 `需求.md`；代码实现已经改变架构、目录、模块边界、接口、存储、构建或运行方式时，必须同步更新 `架构.md`。
- `需求.md` 发生用户明确要求的新增、删除、修改后，必须同步评估是否需要更新 `架构.md`。
- `架构.md` 发生目录边界、技术栈、接口规范、存储规范、构建打包或运行边界调整后，必须同步评估是否需要更新 `需求.md`。
...
- 不允许把用户新提到但 `需求.md` 未覆盖的需求只写入 `AGENTS.md`、`架构.md` 或代码。
