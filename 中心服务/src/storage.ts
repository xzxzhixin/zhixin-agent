...
export class CenterStorage {
  ...
  private readonly directoryMap: Record<string, string>;
  ...
  getCenterDirectoryNames(): readonly string[] {
  ...
  async readProviders(): Promise<ProviderConfig[]> {
    ...
    return this.repository.readJson<ProviderConfig[]>(this.providerFilePath(), []);
  }
  ...
  async readRuntimes(): Promise<RuntimeConfig[]> {
    // runtimes：运行环境配置保存在“运行环境/runtimes.json”。
    return this.repository.readJson<RuntimeConfig[]>(this.runtimeFilePath(), []);
  }

  // saveRuntimes：保存运行环境配置列表。
  async saveRuntimes(runtimes: RuntimeConfig[]): Promise<void> {
    // writeJson：统一由中心服务持久化，避免多端直接写文件。
    await this.repository.writeJson(this.runtimeFilePath(), runtimes);
  }

  // readProxies：读取网络代理配置列表。
  async readProxies(): Promise<NetworkProxyConfig[]> {
    // proxies.json：网络代理配置保存在中心目录“供应商”下，供供应商请求复用。
    return this.repository.readJson<NetworkProxyConfig[]>(this.proxyFilePath(), []);
  }

  // saveProxies：保存网络代理配置列表。
  async saveProxies(proxies: NetworkProxyConfig[]): Promise<void> {
    // writeJson：代理可见配置不包含用户名和密码明文。
    await this.repository.writeJson(this.proxyFilePath(), proxies);
  }

  // readLocalConfig：读取中心服务本机配置。
  async readLocalConfig(): Promise<CenterServiceLocalConfig> {
    // defaultConfig：端口和中心目录来自当前启动配置。
    const defaultConfig = this.createDefaultLocalConfig();
    // readJson：配置文件缺失时返回默认结构。
    return this.repository.readJson<CenterServiceLocalConfig>(this.localConfigFilePath(), defaultConfig);
  }

  // saveLocalConfig：保存中心服务本机配置。
  async saveLocalConfig(config: CenterServiceLocalConfig): Promise<void> {
    // writeJson：本机配置属于配置写，允许整体覆盖。
    await this.repository.writeJson(this.localConfigFilePath(), config as unknown as JsonRecord);
  }

  // readClientPreferences：读取按客户端类型保存的通知和执行模式。
  async readClientPreferences(): Promise<ClientPreferenceState[]> {
    // readJson：客户端偏好保存在会话目录下的 client-preferences.json。
    return this.repository.readJson<ClientPreferenceState[]>(
      this.clientPreferenceFilePath(),
      this.createDefaultClientPreferences(),
    );
  }

  // saveClientPreferences：保存客户端偏好配置。
  async saveClientPreferences(preferences: ClientPreferenceState[]): Promise<void> {
  ...
  async readProjects(): Promise<ProjectRegistration[]> {
    // projects.json：项目状态归属于中心目录“会话”。
    return this.repository.readJson<ProjectRegistration[]>(this.projectsFilePath(), []);
  }

  // saveProjects：保存项目登记列表。
  async saveProjects(projects: ProjectRegistration[]): Promise<void> {
    // writeJson：项目路径、别名和最近打开时间由中心服务统一保存。
    await this.repository.writeJson(this.projectsFilePath(), projects);
  }

  // readSessions：读取会话列表。
  async readSessions(): Promise<ConversationSession[]> {
    // sessions.json：普通会话和项目会话共用会话索引。
    return this.repository.readJson<ConversationSession[]>(this.sessionsFilePath(), []);
  }

  // saveSessions：保存会话列表。
  async saveSessions(sessions: ConversationSession[]): Promise<void> {
    // writeJson：会话索引整体写入，消息内容独立按会话保存。
    await this.repository.writeJson(this.sessionsFilePath(), sessions);
  }

  // readMessages：读取指定会话消息。
  async readMessages(sessionId: string): Promise<ConversationMessage[]> {
    // messagesFilePath：每个会话独立消息文件，减少单文件膨胀。
    return this.repository.readJson<ConversationMessage[]>(this.messagesFilePath(sessionId), []);
  }
  ...
  async readAgents(): Promise<AgentDefinition[]> {
    // agents.json：保存主智能体和团队智能体结构化定义。
    return this.repository.readJson<AgentDefinition[]>(this.agentsFilePath(), this.createDefaultAgents());
  }
  ...
  async readCollaborations(): Promise<AgentCollaborationRecord[]> {
    // collaborations.json：记录管线和群聊协作状态。
    return this.repository.readJson<AgentCollaborationRecord[]>(this.collaborationsFilePath(), []);
  }

  // saveCollaborations：保存智能体协作记录。
  async saveCollaborations(records: AgentCollaborationRecord[]): Promise<void> {
    // writeJson：协作记录用于 UI 查看、介入和管理。
    await this.repository.writeJson(this.collaborationsFilePath(), records);
  }

  // readTasks：读取任务记录列表。
  async readTasks(): Promise<TaskRecord[]> {
    // tasks.json：任务记录保存当时运行环境快照。
    return this.repository.readJson<TaskRecord[]>(this.tasksFilePath(), []);
  }

  // resolveRuntimeSelection：为插件、MCP、skill 或命令任务选择运行环境。
  async resolveRuntimeSelection(runtimeType: RuntimeType, runtimeId?: string): Promise<RuntimeExecutionSelection> {
  ...
  // saveTasks：保存任务记录列表。
  async saveTasks(tasks: TaskRecord[]): Promise<void> {
    // writeJson：任务状态由中心服务统一更新。
    await this.repository.writeJson(this.tasksFilePath(), tasks);
  }

  // readExtensions：读取扩展能力清单。
  async readExtensions(): Promise<ExtensionManifest[]> {
    // extensions.json：插件、MCP 和 skill 使用统一索引。
    return this.repository.readJson<ExtensionManifest[]>(this.extensionsFilePath(), []);
  }

  // readExtensionCalls：读取扩展能力调用记录。
  async readExtensionCalls(): Promise<ExtensionCallRecord[]> {
    // calls.json：所有插件、MCP 和 skill 调用统一进入审计文件。
    return this.repository.readJson<ExtensionCallRecord[]>(this.extensionCallsFilePath(), []);
  }

  // appendExtensionCall：追加扩展能力调用记录。
  async appendExtensionCall(record: ExtensionCallRecord): Promise<void> {
  ...
  async saveExtensions(extensions: ExtensionManifest[]): Promise<void> {
  ...
  async readMcpConfig(): Promise<McpConfigFile> {
    // mcp.json：根字段固定为 mcpServers。
    return this.repository.readJson<McpConfigFile>(this.mcpConfigFilePath(), {
      mcpServers: {},
    });
  }
  ...
  async saveMcpConfig(config: McpConfigFile): Promise<void> {
    ...
    await this.repository.writeJson(this.mcpConfigFilePath(), config as unknown as JsonRecord);
  }

  // readNotifications：读取通知事件列表。
  async readNotifications(): Promise<NotificationEvent[]> {
    // notifications.json：保存中心服务生成并同步过的通知事件。
    return this.repository.readJson<NotificationEvent[]>(this.notificationsFilePath(), []);
  }

  // readPendingMessages：读取等待用户确认的本地排队消息。
  async readPendingMessages(): Promise<PendingMessageRecord[]> {
    // pending-messages.json：恢复连接后不能自动发送，只能展示给用户确认。
    return this.repository.readJson<PendingMessageRecord[]>(this.pendingMessagesFilePath(), []);
  }

  // savePendingMessages：保存等待用户确认的本地排队消息。
  async savePendingMessages(messages: PendingMessageRecord[]): Promise<void> {
  ...
  async readAttachmentContent(attachmentId: string): Promise<Buffer | null> {
  ...
  async saveNotifications(events: NotificationEvent[]): Promise<void> {
  ...
  async readMemories(): Promise<MemoryReadResult[]> {
  ...
  async migrateCenterDirectory(targetDirectory: string): Promise<void> {
  ...
  async readUsageRecords(): Promise<ModelUsageRecord[]> {
    // usage.json：用量统计只追加记录，不回改历史。
    return this.repository.readJson<ModelUsageRecord[]>(this.usageFilePath(), []);
  }

  // appendUsageRecord：追加模型调用用量记录。
  async appendUsageRecord(record: ModelUsageRecord): Promise<void> {
    // records：读取已有记录后追加。
  ...
  async summarizeUsage(filters: {
    ...
    const groups = new Map<string, ModelUsageSummary>();
  ...
  createProviderDraft(): ProviderConfig {
  ...
  private async ensureSeedFiles(): Promise<void> {
    ...
    await this.ensureJsonFile(this.localConfigFilePath(), this.createDefaultLocalConfig() as unknown as JsonRecord);
  ...
  private createRuntimeTemplates(): RuntimeConfig[] {
    // types：需求要求内置的常用运行环境类型，包含 Maven。
    const types: RuntimeConfig["type"][] = ["node", "python", "java", "maven", "git"];
  ...
  private createDefaultLocalConfig(): CenterServiceLocalConfig {
    // now：配置初始化时间。
    const now = new Date().toISOString();
  ...
  private createDefaultClientPreferences(): ClientPreferenceState[] {
    // clientTypes：需求规定的客户端类型集合。
    const clientTypes: ClientType[] = ["desktop", "web", "idea"];
    // map：默认执行模式为全自动，通知默认开启但仅作配置保存。
    return clientTypes.map((clientType) => ({
      clientType,
      executionMode: "full-auto" as ExecutionMode,
      notificationConfig: this.createDefaultNotificationConfig(clientType),
    }));
  }

  // createDefaultNotificationConfig：生成单个客户端类型通知配置。
  private createDefaultNotificationConfig(clientType: ClientType): NotificationConfig {
  ...
  private createDefaultAgents(): AgentDefinition[] {
    // now：智能体定义初始化时间。
    const now = new Date().toISOString();
  ...
  private providerFilePath(): string {
  ...
  private collaborationsFilePath(): string {
    // join：协作记录归属于会话状态。
  ...
  private async collectAllFiles(directoryPath: string): Promise<string[]> {
    // entries：目录不存在时返回空列表。
    const entries = await this.repository.listDirectory(directoryPath);
    // results：递归文件列表。
    const results: string[] = [];
  ...
  private async ensureJsonFile(filePath: string, value: JsonRecord): Promise<void> {
