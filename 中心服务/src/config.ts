...
export interface CenterServiceConfig {
  // port：HTTP 服务监听端口，默认 8866。
  port: number;
  // centerDirectory：中心目录绝对路径，所有固化数据都在这里。
  centerDirectory: string;
}

// readCenterServiceConfig：从环境变量读取中心服务配置，缺省时使用需求约定默认值。
export function readCenterServiceConfig(): CenterServiceConfig {
...
export function buildCenterDirectoryMap(centerDirectory: string): Record<string, string> {
