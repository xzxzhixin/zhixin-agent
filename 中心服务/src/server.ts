...
export class CenterHttpServer {
  ...
  // storage：中心目录文件存储能力。
  private readonly storage: CenterStorage;
  // logger：中心服务日志写入能力。
  ...
  private readonly webSessions: WebSessionManager;
  // lockFilePath：中心服务启动锁路径，按中心目录隔离。
  private readonly lockFilePath: string;

  // constructor：注入配置和存储依赖。
  constructor(config: CenterServiceConfig, storage: CenterStorage) {
  ...
  private async handleHealth(_request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  private async handleDirectories(_request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  private async handleConfig(_request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  private async handleAuthStatus(request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  private async handleUpdateConfig(request: IncomingMessage, response: ServerResponse): Promise<void> {
    // body：桌面端设置页提交的本机配置。
    const body = await this.readJsonBody<Partial<CenterServiceLocalConfig> & { webPassword?: string }>(request);
    // current：未传字段沿用当前配置，避免只更新单项时丢失其他配置。
    const current = await this.storage.readLocalConfig();
    // next：端口修改后需要重启生效，配置中仍先保存目标值。
    const next: CenterServiceLocalConfig = {
      ...current,
  ...
  private async handleLogin(request: IncomingMessage, response: ServerResponse): Promise<void> {
    ...
    const body = await this.readJsonBody<LoginRequest>(request);
  ...
  private async handleProviders(_request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  private async handleSaveProvider(request: IncomingMessage, response: ServerResponse): Promise<void> {
    // body：供应商配置写入请求，API Key 只在本次请求中出现。
    const body = await this.readJsonBody<ProviderUpsertRequest>(request);
    // providers：读取现有供应商列表。
    const providers = await this.storage.readProviders();
    // now：更新时间使用 ISO 8601。
    ...
    if (body.apiKey) {
      await this.storage.getRepository().writeText(
        this.providerSecretPath(saved.id),
        `${createApiKeyMarker(body.apiKey)}\n`,
        "config-write",
      );
    }
    // next：替换已有供应商或追加新供应商。
  ...
  private async handleDeleteProvider(request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  private async handleProxies(_request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  private async handleSaveProxy(request: IncomingMessage, response: ServerResponse): Promise<void> {
    // body：代理配置写入请求，用户名和密码允许为空。
    const body = await this.readJsonBody<NetworkProxyUpsertRequest>(request);
    // proxies：读取现有代理配置。
    const proxies = await this.storage.readProxies();
    // existing：修改时按 id 查找已有代理。
    const existing = body.id ? proxies.find((proxy) => proxy.id === body.id) : undefined;
    // saved：客户端可见配置不包含用户名和密码明文。
    const saved: NetworkProxyConfig = {
      id: existing?.id ?? randomUUID(),
      name: body.name,
  ...
  private async handleDeleteProxy(request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  private async handleDefaultProxy(_request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  private async handleRefreshProvider(request: IncomingMessage, response: ServerResponse): Promise<void> {
    // id：按供应商 ID 刷新。
    const id = this.getQueryParam(request, "id");
  ...
  private async handleRuntimes(_request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  private async handleSaveRuntime(request: IncomingMessage, response: ServerResponse): Promise<void> {
    // body：运行环境配置请求。
    const body = await this.readJsonBody<RuntimeUpsertRequest>(request);
    // runtimes：读取现有运行环境配置。
    const runtimes = await this.storage.readRuntimes();
    // existing：修改时按 id 匹配。
  ...
  private async handleDeleteRuntime(request: IncomingMessage, response: ServerResponse): Promise<void> {
    // id：运行环境 ID 来自 query。
    const id = this.getQueryParam(request, "id");
    // runtimes：过滤目标环境。
    const runtimes = await this.storage.readRuntimes();
  ...
  private async handleClientPreferences(_request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  private async handleSaveClientPreference(request: IncomingMessage, response: ServerResponse): Promise<void> {
    // body：单个客户端类型偏好配置。
    const body = await this.readJsonBody<ClientPreferenceState>(request);
    // preferences：读取现有偏好。
    const preferences = await this.storage.readClientPreferences();
  ...
  private async handleProjects(_request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  private async handleRegisterProject(request: IncomingMessage, response: ServerResponse): Promise<void> {
    // body：IDE 插件或 UI 端上报的项目身份。
    const body = await this.readJsonBody<ProjectRegistration>(request);
    // projects：读取现有项目。
    const projects = await this.storage.readProjects();
  ...
  private async handleSessions(request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  private async handleCreateSession(request: IncomingMessage, response: ServerResponse): Promise<void> {
    ...
    const body = await this.readJsonBody<Partial<ConversationSession>>(request);
    // now：会话创建与更新时间。
    const now = new Date().toISOString();
  ...
  private async handleAppendMessage(request: IncomingMessage, response: ServerResponse): Promise<void> {
    // body：消息内容、附件和上下文引用。
    const body = await this.readJsonBody<Partial<ConversationMessage> & { sessionId: string }>(request);
    // message：补齐中心服务统一字段。
    ...
    const nextSessions = sessions.map((session) => (session.id === body.sessionId
      ? {
        ...session,
        status: message.role === "user" ? "working" as const : session.status,
  ...
  private async handleAgents(_request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  private async handleSaveAgent(request: IncomingMessage, response: ServerResponse): Promise<void> {
    // body：智能体定义。
    const body = await this.readJsonBody<Partial<AgentDefinition>>(request);
    // agents：读取现有智能体。
    const agents = await this.storage.readAgents();
  ...
  private async handleDeleteAgent(request: IncomingMessage, response: ServerResponse): Promise<void> {
    // id：智能体 ID 来自 query。
    const id = this.getQueryParam(request, "id");
    // agents：读取现有智能体。
    const agents = await this.storage.readAgents();
  ...
  // handleTasks：读取任务记录。
  private async handleTasks(_request: IncomingMessage, response: ServerResponse): Promise<void> {
    // tasks：包含任务状态和运行环境快照。
  ...
  private async handleExtensions(_request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  private async handleMcpConfig(request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  private async handleSaveMcpConfig(request: IncomingMessage, response: ServerResponse): Promise<void> {
    // body：MCP 配置文件结构。
    const body = await this.readJsonBody<McpConfigFile>(request);
  ...
  private async handleNotifications(_request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  private async handleUploadAttachment(request: IncomingMessage, response: ServerResponse): Promise<void> {
    // body：图片附件通过 base64 提交，由中心服务写入会话附件目录。
    const body = await this.readJsonBody<AttachmentUploadRequest>(request);
  ...
  private async handleReadAttachment(request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  private async handleRealtimeSync(_request: IncomingMessage, response: ServerResponse): Promise<void> {
    // snapshot：首版使用轮询快照，覆盖项目、聊天、消息状态、智能体、任务、供应商和扩展能力。
    const snapshot: RealtimeSyncSnapshot = {
  ...
  private async handlePendingMessages(_request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  private async handleSavePendingMessage(request: IncomingMessage, response: ServerResponse): Promise<void> {
    // body：客户端断线时保存的待确认消息。
    const body = await this.readJsonBody<Partial<PendingMessageRecord> & { sessionId: string; content: string }>(request);
    // messages：读取已有排队消息。
    const messages = await this.storage.readPendingMessages();
    // message：状态固定为待用户确认，避免恢复连接后静默发送。
    const message: PendingMessageRecord = {
      id: body.id ?? randomUUID(),
      clientType: body.clientType ?? "desktop",
      sessionId: body.sessionId,
      content: body.content,
  ...
  private async handleCollaborations(_request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  private async handleSaveCollaboration(request: IncomingMessage, response: ServerResponse): Promise<void> {
    // body：协作记录请求。
    const body = await this.readJsonBody<Partial<AgentCollaborationRecord> & { sessionId: string }>(request);
    // records：读取已有协作。
    const records = await this.storage.readCollaborations();
    // now：创建和更新时间。
    const now = new Date().toISOString();
    // record：管线或群聊协作记录。
    const record: AgentCollaborationRecord = {
      id: body.id ?? randomUUID(),
  ...
  private async handleMemories(_request: IncomingMessage, response: ServerResponse): Promise<void> {
    // memories：记忆读取可以并发执行。
  ...
  private async handleAppendMemory(request: IncomingMessage, response: ServerResponse): Promise<void> {
    // body：调用方必须提交完整一轮对话摘要，中心服务不把工具调用拆成多段记忆。
    const body = await this.readJsonBody<MemoryRecord>(request);
  ...
  private async handleAppendUserMemory(request: IncomingMessage, response: ServerResponse): Promise<void> {
    // body：用户记忆文本。
    const body = await this.readJsonBody<{ content: string }>(request);
  ...
  private async handleRuntimeSelection(request: IncomingMessage, response: ServerResponse): Promise<void> {
    // body：运行环境类型和可选 ID。
    const body = await this.readJsonBody<RuntimeExecutionRequest>(request);
  ...
  private async handleUsageSummary(request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  private async handleAppendUsage(request: IncomingMessage, response: ServerResponse): Promise<void> {
    // body：用量记录必须携带调用时供应商、模型和项目快照。
    const body = await this.readJsonBody<ModelUsageRecord>(request);
  ...
  private async handleSaveExtension(request: IncomingMessage, response: ServerResponse): Promise<void> {
    // body：插件、MCP 或 skill 清单。
    const body = await this.readJsonBody<Partial<ExtensionManifest> & { name: string; type: ExtensionManifest["type"] }>(request);
    // extensions：读取已有扩展索引。
  ...
  private async handleDeleteExtension(request: IncomingMessage, response: ServerResponse): Promise<void> {
    // id：扩展 ID 来自 query。
    const id = this.getQueryParam(request, "id");
    // extensions：读取索引。
    const extensions = await this.storage.readExtensions();
  ...
  private async handleAppendExtensionCall(request: IncomingMessage, response: ServerResponse): Promise<void> {
    // body：调用记录。
    const body = await this.readJsonBody<ExtensionCallRecord>(request);
    // record：补齐 ID 和时间。
    const record: ExtensionCallRecord = {
      ...body,
      id: body.id || randomUUID(),
  ...
  private async handleExtensionCalls(_request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  private async handlePluginHtml(_request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  private getQueryParam(request: IncomingMessage, name: string): string {
  ...
  private getOptionalQueryParam(request: IncomingMessage, name: string): string | undefined {
    // value：复用固定 query 解析。
  ...
  private async isAuthorizedRequest(request: IncomingMessage): Promise<boolean> {
  ...
  private async createAuthStatus(request: IncomingMessage): Promise<AuthStatusResponse> {
    ...
    const validation: {
      valid: boolean;
      expiresAt?: string;
    } = token ? this.webSessions.verify(token) : { valid: false };
  ...
  private readCookie(request: IncomingMessage, name: string): string {
  ...
  private createWebSessionCookie(request: IncomingMessage, token: string, maxAgeSeconds: number): string {
  ...
  private providerSecretPath(providerId: string): string {
  ...
  private proxySecretPath(proxyId: string): string {
    // directories：敏感标记保存在供应商目录，客户端不直接读取。
    const directories = this.storage.getDirectoryMap();
  ...
  private async refreshProviderCapabilities(provider: ProviderConfig): Promise<ProviderConfig> {
    // now：刷新更新时间。
    const now = new Date().toISOString();
    ...
    if (provider.type === "openai-compatible") {
      try {
        ...
        const refreshedModels = payload.data?.map((item) => item.id).filter((id): id is string => Boolean(id)) ?? [];
  ...
  private async classifyProxyReadiness(provider: ProviderConfig): Promise<string | undefined> {
  ...
  private formatProviderFailure(kind: ProxyFailureKind, message: string): string {
    // labels：按需求区分代理连接、代理认证、供应商连接和供应商接口失败。
    const labels: Record<ProxyFailureKind, string> = {
  ...
  private async applyEnabledProviderDefaults(providers: ProviderConfig[]): Promise<{
  ...
  private async resolveDefaultAgent(agentId?: string): Promise<AgentDefinition> {
    // agents：读取智能体定义。
    const agents = await this.storage.readAgents();
  ...
  private async persistAgentMarkdown(agent: AgentDefinition): Promise<void> {
  ...
  private async readExtensionsWithProjectScope(): Promise<ExtensionManifest[]> {
    ...
    // merged：同名同类型能力项目级优先。
    const merged = new Map<string, ExtensionManifest>();
    // addGlobal：先放全局。
  ...
  private async scanProjectExtensions(): Promise<ExtensionManifest[]> {
  ...
  private async readProjectMcpConfig(projectId: string): Promise<McpConfigFile> {
    ...
    const configs = await Promise.all(entries.filter((entry) => entry.endsWith(".json")).map((entry) => {
      ...
      return this.storage.getRepository().readJson<McpConfigFile>(filePath, {
        mcpServers: {},
      });
    }));
    // merged：同一个项目目录中后读取的同名配置覆盖前者。
    return configs.reduce<McpConfigFile>((current, item) => ({
      mcpServers: {
        ...current.mcpServers,
        ...item.mcpServers,
      },
